t1<-0;t2<-1;p1<-0;p2<-0;s2<-1;res<-100
...
for(n in 1:n.bridges){
  ...
  for(t.ind in ord){
    ...
    mat[t.ind,n]<-rnorm(1,
                        ...
                        sqrt(s2*(ts[t.ind]-ts[last.t.ind])*(ts[next.t.ind]-ts[t.ind])/(ts[next.t.ind]-ts[last.t.ind])))
  }
}
...
hist(apply(mat,2,function(ii) sum(ii*diff(ts)[1])))
...
#interval.
matplot(mat[,sample(1000,1000)],type='l',col=rgb(0,0,0,0.1),lty=1)
yy<-s2*(ts-t1)*(t2-ts)/(t2-t1)
plot(as.vector(apply(mat,1,quantile,probs=c(0.16,0.84)))~rep(ts,each=2),ylim=c(-1,1))
lines(sqrt(1*yy)~ts,type='l')
lines(-1*sqrt(yy)~ts,type='l')
