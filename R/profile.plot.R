...
#' @export
profile.plot<-function(fit,select='.',separate.R=F,separate.X=F,separate.dev=F,together=F,dens=F,alpha=NA,...){
  ...
  #dealing with breaks/bandwidths...
  if(together){
    if(dens){
  ...
  #to store histograms/density functions before-hand would be best approach, I believe...
  ...
  for(i in names(args.master)){
    ...
    for(j in incl.names){
      for(k in 1:dim(chains)[3]){
        if(dens){
          graphs[[j]][[k]]<-do.call(density,
                                    c(list(chains[,j,k]),
                                      args.master[[i]]))
        }else{
          graphs[[j]][[k]]<-do.call(hist,
                                    c(list(chains[,j,k]),
                                      plot=F,
                                      args.master[[i]][names(args.master[[i]])!='plot']))
        }
      }
    }
  }
  ...
  if(together){
    ...
    #have to figure out a way to deal with frequency vs. density --> decided to just default to density
    ...
    do.call(plot,c(x=0,
                   ...
                   xlim=list(xlim),
                   ylim=list(ylim),
                   list(...)[!(names(list(...))%in%c('xlab','ylab','xlim','ylim','type','pch','col','adj'))&
  ...
  for(i in names(args.master)){
    ...
    if(!together){
      ...
      do.call(plot,c(x=0,
                     ...
                     pch=1,
                     col='white',
                     args.master[[i]][!(names(args.master[[i]])%in%c('type','pch','col','adj'))]))
    }
      ...
      for(k in 1:dim(chains)[3]){
        if(dens){
          do.call(polygon,c(x=list(graphs[[incl.names[j]]][[k]]$x),
                            ...
                            args.master[[i]][!(names(args.master[[i]])%in%c('density','angle','col','add','adj'))]))
        }else{
          do.call(plot,c(x=list(graphs[[incl.names[j]]][[k]]),
                         ...
                         col=args.master[[i]]$col[[j]],
                         add=T,
                         args.master[[i]][!(names(args.master[[i]])%in%c('freq','density','angle','col','add','adj'))]))
        }
      }
    }
  }
