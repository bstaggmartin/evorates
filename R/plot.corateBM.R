...
plot.corateBM<-function(sim,tree,cols=c('deepskyblue','darkgray','brown1'),
                        phylogram=T,val.range=range(sim$R),res=100,...){
  if(hasArg(xlab)){
    xlab<-list(...)$xlab
  }else{
    xlab<-'time'
  ...
  if(phylogram){
    ...
    if(length(sim$X)==n){
      ...
      segments(x0=node.depth.edgelength(tree)[tree$edge[,1]],x1=node.depth.edgelength(tree)[tree$edge[,2]],
               y0=c(sim$X,tmp)[tree$edge[,1]],y1=c(sim$X,tmp)[tree$edge[,2]],col=colvec,xlab,ylab,...)
    }else{
      plot(sim$X[c(tree$tip.label,1:tree$Nnode)]~node.depth.edgelength(tree),col='white',...)
      segments(x0=node.depth.edgelength(tree)[tree$edge[,1]],x1=node.depth.edgelength(tree)[tree$edge[,2]],
               y0=sim$X[tree$edge[,1]],y1=sim$X[tree$edge[,2]],col=colvec,xlab,ylab,...)
    }
  }else{
