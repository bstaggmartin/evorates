...
#' @param node.groups A list of numeric or character vectors specifying clades for
...
#' details).
#' @param edge.groups A list of numeric vectors specifying edge indices in \code{fit$call$tree} for 
...
#' @param simplify \code{TRUE} or \code{FALSE}: should the resulting \code{param_block} array(s) be 
...
#' @param remove.trend \code{TRUE} or \code{FALSE}: should rates be detrended prior to calculating
...
#' @param geometric \code{TRUE} or \code{FALSE}: should rates be geometrically averaged on the natural
...
#' @param log \code{TRUE} or \code{FALSE}: should rates be returned on the natural log scale? Defaults to
#' \code{TRUE} for consistency, since branchwise rates are generally on the natural log scale.
#' @param partial.match \code{TRUE} or \code{FALSE}: are partial matches between any strings in
#' \code{node.group} and labels in \code{fit$call$tree} allowed? Defaults to \code{TRUE}.
#' @param keep.R \code{TRUE} or \code{FALSE}: should the branchwise rates used to calculate background
...
#' 
#' 
#' @return Typically an array of class "\code{param_block}" with a \code{param_type} set to whatever
...
#' 
#' 
#' @seealso more information on both detrending and averaging branchwise rates is available in the
...
get.bg.rate<-function(fit,
                      ...
                      keep.R=FALSE){
  tree<-fit$call$tree
  if(!is.list(node.groups)){
    node.groups<-list(node.groups)
  }
  if(length(node.groups)==0&length(edge.groups)==0){
    edge.groups<-NULL
  }
  if(!is.list(edge.groups)){
    edge.groups<-list(edge.groups)
  }
  node.groups<-setNames(lapply(node.groups,
                               function(ii) if(length(ii)) get.clade.edges(tree,ii,partial.match=partial.match)),
                        names(node.groups))
  edge.groups<-setNames(lapply(edge.groups,unique),
                        names(edge.groups))
  edge.groups<-c(node.groups,edge.groups)
  edge.groups[!lengths(edge.groups)]<-list(seq_len(Nedge(fit)))
