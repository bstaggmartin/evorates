...
transformed parameters {
  ...
  real tmp_prior; //log prior (- cauchy prior params)
  ...
  //trait data
    ...
    lik = lik_power * (sum(LL) - 0.5 * (n * log_determinant(Xcov) + 
...
generated quantities{
  ...
  {real tmp;
  for(i in 1:k){
    tmp += cauchy_lpdf(X0[i] | X0_prior_mu[i], X0_prior_sig[i]);
  }
  if(!constr_Rsig2){
    tmp += cauchy_lpdf(Rsig2[1] | 0, Rsig2_prior);
  }
  if(!constr_Rmu){
    tmp += cauchy_lpdf(Rmu[1] | Rmu_prior_mu, Rmu_prior_sig);
  }
  prior = tmp_prior + cauchy_lpdf(R0 | R0_prior_mu, R0_prior_sig) + tmp;}
}
